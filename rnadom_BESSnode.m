function rnadom_BESSnode( fn )
%UNTITLED2 Summary of this function goes here
%   Detailed explanation goes here
clc
[~,~,customernodes] = xlsread( fn, 'CONSUMER' );
customernodes=customernodes(2:end,:);
T=(customernodes(:,1));
length(T);

for i=1:20
    index=randi(length(T));
    BESS_Nodes(i)=T(index);
end
BESS_Nodes1=BESS_Nodes';

SAIDIARRAY=cell(length(BESS_Nodes1),1);
[~,~,CUST] = xlsread( fn, 'CONSUMER' );
[~,~,SW] = xlsread(fn, 'SW' );
[~,~,LINE] = xlsread( fn, 'LINE' );
[~,~,LINETYPE] = xlsread( fn, 'LINETYPE' );
[~,~,BACKUP] = xlsread( fn, 'SOURCENODES' );
...
data1=['AABJA';'AABJJ';'AACAB';'AABIF';'AACEC';'AAAFC';'AACDD';'AAAGE';'AABCB';'AAAIJ';'AACGB';'AAADH';'AAAEE';'AABCH';'AACGD';'AAACI'];
MostCustNodes=cellstr(data1);
data2=['AACCA';'AAAEJ';'AACFF';'AACCD';'AABBE';'AAAFJ';'AABGG';'AAAEC';'AABCB';'AAAHE';'AAAEA';'AAAED';'AABEG';'AAADB';'AAABD';'AACHB'];
mostFaultyNodes=cellstr(data2);
increment=[0:10:140];
faulttime=180+(180*increment(1,:))/100;
faulttime=faulttime';

Faultrate_Road=.023+(.023*increment(1,:))/100;
Faultrate_Road=Faultrate_Road';

Faultrate_Field=.01+(.01*increment(1,:))/100;
Faultrate_Field=Faultrate_Field';

Faultrate_Forest=.07+(.07*increment(1,:))/100;
Faultrate_Forest=Faultrate_Forest';

%Declear the array of nodes in which BESSs will be installed
SAIDIARRAY=cell(length(MostCustNodes),1);
SAIFIARRAY=cell(length(MostCustNodes),1);
for AddedBesIndex=1:length(MostCustNodes)    
size(LINE);
BESS_nodeIssmember=ismember(CUST(:,1),MostCustNodes(AddedBesIndex,1));
Time=(cell2mat(CUST(BESS_nodeIssmember,3))/100)*3600;
CUST{BESS_nodeIssmember,6}=Time;
CUST{BESS_nodeIssmember,5}=10;


%LINETYPE{1,8}=faulttime(AddedBesIndex,1);
%LINETYPE{2,8}=faulttime(AddedBesIndex,1);
%LINETYPE{3,8}=faulttime(AddedBesIndex,1);

%LINETYPE{1,7}=Faultrate_Road(AddedBesIndex,1);
%LINETYPE{2,7}=Faultrate_Field(AddedBesIndex,1);
%LINETYPE{3,7}=Faultrate_Forest(AddedBesIndex,1);
...
SAIDIARRAY{AddedBesIndex}=SAIDI_Final;
...
for line = 1:size(LINE,1)
    ...
    %BESS nodes which are able to support the loads   
     Min_SW_time=min(cell2mat(SW(:,4)));             
     BESS_Nodes=cell(size(CUST(:,6)));
      for s=1:length(CUST(:,6))
...
SAIFIARRAY{AddedBesIndex}=Calculate_SAIFI;
end
SAIDIARRAY=cell2mat(SAIDIARRAY)
SAIFIARRAY=cell2mat(SAIFIARRAY)
